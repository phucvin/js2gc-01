...
(local $temp_3 (ref $CallSite))
(local $cache_4 (ref null $CallSite))
...
(call $console_log (block (result anyref)
             ...
             (call $get_field_cached (ref.cast (ref $Object) (local.get $user_obj)) (ref.as_non_null (local.get $cache_2)) (i32.const 1))
          ))
(drop)
(call $console_log (block (result anyref)
             (if (ref.is_null (local.get $cache_4))
               (then (local.set $cache_4 (call $new_callsite)))
             )
             (call $get_field_cached (ref.cast (ref $Object) (local.get $user_obj)) (ref.as_non_null (local.get $cache_4)) (i32.const 1))
          ))
  )
)