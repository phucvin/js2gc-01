...
(block $break
  (loop $continue
    (br_if $break (i32.eqz (i31.get_s (ref.cast (ref i31) (call $less_than (local.get $user_i) (ref.i31 (i32.const 10)))))))
(local.tee $user_sum (call $add (local.get $user_sum) (call $get_field_cached (ref.cast (ref $Object) (local.get $user_obj)) (global.get $site_0) (i32.const 0))))
(drop)
(block (result anyref)
                  (local.set $temp_1 (local.get $user_i))
                  (local.set $user_i (call $add (local.get $temp_1) (ref.i31 (i32.const 1))))
                  (local.get $temp_1)
              )
(drop)
    (br $continue)
