...
export function compileCallExpression(expr: ts.CallExpression, ctx: CompilationContext, dropResult: boolean): string {
    const options = ctx.getOptions();
    const enableIC = options.enableInlineCache !== false;
    const fallback = (code: string) => dropResult ? `(drop ${code})` : code;

    // Direct console.log is handled in dispatcher
