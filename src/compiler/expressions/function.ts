...
import { CompilationContext, registerShape, getPropertyId, registerGlobalCallSite, registerGeneratedFunction } from '../context.ts';
...
export function compileFunctionExpression(
  ...
  closureCtx.getLocals().forEach((type, name) => {
    const isParam = node.parameters.some(
      (p) => ts.isIdentifier(p.name) && `$user_${p.name.text}` === name,
    );
