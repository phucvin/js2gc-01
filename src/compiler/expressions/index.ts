...
import { compileNumericLiteral, compileStringLiteral, compileObjectLiteral } from './literal.ts';
import { compileBinaryExpression, compilePostfixUnaryExpression } from './binary.ts';
import { compileCallExpression } from './call.ts';
import { compileFunctionExpression, resetClosureCounter } from './function.ts';
...
function compileExpressionValue(expr: ts.Expression, ctx: CompilationContext, dropResult: boolean): string {
  ...
  if (ts.isNumericLiteral(expr)) {
      return compileNumericLiteral(expr, ctx, dropResult);
  } else if (ts.isStringLiteral(expr)) {
      return compileStringLiteral(expr, ctx, dropResult);
      ...
      return compileObjectLiteral(expr, ctx, dropResult);
