...
async function runBenchmark() {
  ...
  const jsFiles = findFiles(benchmarkDir, '.js');
  ...
  for (const file of jsFiles) {
    ...
    const compileAndRunWasm = async (enableIC: boolean): Promise<number | string> => {
      ...
      const watPath = path.join(
        benchmarkDir,
        `${file.replace('.js', '')}.${enableIC ? 'ic' : 'no_ic'}.wat`,
      );
      ...
      const wasmPath = path.join(
        benchmarkDir,
        `${file.replace('.js', '')}.${enableIC ? 'ic' : 'no_ic'}.wasm`,
      );
...
function updateReadme(results: BenchmarkResult[]) {
  ...
  results.forEach((r) => {
    const ratioIC =
      typeof r.wasmICDuration === 'number'
        ? (r.wasmICDuration / r.jsDuration).toFixed(2)
        : 'N/A';
    const ratioNoIC =
      typeof r.wasmNoICDuration === 'number'
        ? (r.wasmNoICDuration / r.jsDuration).toFixed(2)
        : 'N/A';

    const icVal =
      typeof r.wasmICDuration === 'number' ? r.wasmICDuration.toFixed(4) : r.wasmICDuration;
    const noIcVal =
      typeof r.wasmNoICDuration === 'number' ? r.wasmNoICDuration.toFixed(4) : r.wasmNoICDuration;
